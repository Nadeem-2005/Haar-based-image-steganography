function extractt(stego_img_path, cover_img_path, msg_length)
    ...
    % Apply DWT
    [~, ~, ~, cD_stego] = dwt2(stego_img, 'haar');
    [~, ~, ~, cD_cover] = dwt2(cover_img, 'haar');
    
    % Extract message length from first 16 coefficients
    len_bits = '';
    for i = 1:16
        diff = cD_stego(i) - cD_cover(i);
        if diff > 0.005
            len_bits = [len_bits, '1'];
        else
            len_bits = [len_bits, '0'];   
    ...
    for i = 1:msg_bit_length
        diff = cD_stego(i+16) - cD_cover(i+16);
        if diff > 0.005
            extracted_bits(i) = 1;
        else
            extracted_bits(i) = 0;
    ...
    % Display Images
    figure;
    subplot(1,2,1), imshow(stego_img), title('Stego Image');
    subplot(1,2,2), imshow(cover_img), title('Original Image');
end


extractt('stego_image1.png', 'cover_image1.jpg', length('Hello, World !!'));
extractt('stego_image2.png', 'cover_image2.jpg', length('Hello, India !!'));
extractt('stego_image3.png', 'cover_image3.jpg', length('Hello, Tamil Nadu !!'));
extractt('stego_image4.png', 'cover_image4.jpg', length('Hello, Vellore !!'));